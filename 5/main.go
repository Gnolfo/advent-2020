...
func getID(pass string) int {
	rz := regexp.MustCompile("[FL]")
	ro := regexp.MustCompile("[BR]")
	binStr := ro.ReplaceAllString(rz.ReplaceAllString(pass, "0"), "1")
	i, err := strconv.ParseInt(binStr, 2, 0)
	if err != nil {
		panic(err)
	}
