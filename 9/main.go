...
20. Now, for the next number to be valid, there needs to be some pair of numbers among 1-19, 21-25, or 45 that add up
to it:
...
func main() {
	...
	for i := preamble; i < len(numbers); i++ {
		if !hasAdditivePair(numbers[i], numbers[(i-preamble):i]) {
			fmt.Printf("Line %d has number %d with no matching pair in %d lines before.\n", i, numbers[i], preamble)
			invalid = numbers[i]
			break
		}
	}
	...
	for i := range numbers {
		sumList := findContiguousSum(numbers, invalid, i)
		if sumList != nil {
			fmt.Printf("Contiguous list for %d: %v\n", invalid, sumList)
			sort.Sort(sort.IntSlice(sumList))
			fmt.Printf("Lowest: %d, Highest: %d, Sum: %d\n", sumList[0],
...
func hasAdditivePair(target int, list []int) bool {
	...
	for {
		...
		sum = sorted[startInd] + sorted[endInd]
		if sum == target {
			return true
		}
...
func findContiguousSum(arr []int, target int, start int) []int {
	sum := 0
