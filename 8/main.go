...
func main() {
	...
	var code []string
	var possibleBuggyLines []int
	scanner := bufio.NewScanner(strings.NewReader(string(input)))
	for scanner.Scan() {
		line := scanner.Text()
		code = append(code, line)
		if line[:3] == "jmp" || line[:3] == "nop" {
			possibleBuggyLines = append(possibleBuggyLines, len(code)-1)
		}
	}

	// run the code as-is to get the part 1 answer
	line, acc := runCode(code, -1)

	fmt.Printf("Naive run halted on line %d. Acc: %d\n", line, acc)

	for _, v := range possibleBuggyLines {
		line, acc := runCode(code, v)
		if line < len(code) {
			//		fmt.Printf("Swap failed (halted on line %d. Acc: %d)\n", line, acc)
		} else {
			fmt.Printf("Swap succeeded. executed to line %d. Acc: %d\n", line, acc)
		}
...
func runCode(code []string, swap int) (int, int) {
	executed := make(map[int]struct{})
	ip := 0
	acc := 0
	for {
		// check if we've run this line, if so we halt and return the duplicate line of code and current acc
		_, ok := executed[ip]
		if ok || ip < 0 || ip >= len(code) {
			return ip, acc
		}
		...
		if swap == ip {
			...
			case "nop":
				instr = "jmp" + instr[3:]
			default:
				panic(fmt.Errorf("cannot swap op: %s", instr[:3]))
			}
			if debug {
				fmt.Printf("Swapping instr %d: `%s` -> `%s`\n", ip, old, instr)
			}
		}

		executed[ip] = struct{}{}
		jump, delta := runInstruction(instr)
		ip += jump
		acc += delta
	}
}

func runInstruction(inst string) (int, int) {
	parsed := strings.Split(inst, " ")
	op := parsed[0]
	arg, err := strconv.Atoi(parsed[1])
	if err != nil {
		panic(err)
	}
	if op == "nop" {
	...
	case "acc":
		return 1, arg
	default:
		panic(fmt.Errorf("invalid op: %s", op))
	}
}

func swapLine(code []string, line int) {
	var modifiedCode []string
	copy(modifiedCode, code)
	op := modifiedCode[line][:3]
	switch op {
	case "jmp":
	case "nop":
	default:
		panic(fmt.Errorf("cannot swap op: %s", op))
	}
}
