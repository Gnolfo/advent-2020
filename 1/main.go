...
func findTripleSum(arr []int, target int) []int {
	...
	for {
		...
		sum = target - subarr[0]
		result = findSum(subarr[1:], sum)

		if result != nil {
			return []int{subarr[0], result[0], result[1]}
		}
...
func findSum(arr []int, target int) []int {
	...
	for {
		...
		if sum == target {
			return []int{arr[startInd], arr[endInd]}
		}
